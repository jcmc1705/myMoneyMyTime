...
export default class Transaction {
    ...
    constructor(
        ...
        if (!this.isValidValue(value)) throw new Error('Valor deve ser um número maior que zero!');
        if (!this.isValidTypeTransaction(typeTransaction)) throw new Error('Tipo de transação inválida!');
        if (!this.isValidDescription(description)) throw new Error('Descrição deve possuir entre 3 e 50 caracteres!');
